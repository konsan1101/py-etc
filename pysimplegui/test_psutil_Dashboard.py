...
GRAPH_WIDTH = 240       # each individual graph size in pixels
GRAPH_HEIGHT = 40
ALPHA = .7
...
def main():
    ...
    while True :
        ...
        window['_NET_OUT_TXT_'].update('Net out ' + str(human_size(write_bytes)))
        window['_NET_IN_TXT_'].update('Net In ' + str(human_size(read_bytes)))
        # ----- Disk Graphs -----
        diskio = psutil.disk_io_counters()
        write_bytes = disk_graph_write.graph_value(diskio.write_bytes)
        read_bytes = disk_graph_read.graph_value(diskio.read_bytes)
        window['_DISK_WRITE_TXT_'].update('Disk_Write ' + str(human_size(write_bytes)))
        window['_DISK_READ_TXT_'].update('Disk_Read ' + str(human_size(read_bytes)))
        # ----- CPU Graph -----
