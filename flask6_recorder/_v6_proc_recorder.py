...
class proc_recorder:
    ...
    def __init__(self, name='thread', id='0', runMode='debug', ):
        ...
        json_file = 'recorder_key.json'
        self.inpFPS = 10
        self.outFPS = 10
        res, json_dic = qFunc.getJson(json_file=json_file, )
        if (res == True):
            self.inpFPS = json_dic['inpFPS']
            self.outFPS = json_dic['outFPS']
        else:
            json_dic['inpFPS'] = self.inpFPS
            json_dic['outFPS'] = self.outFPS
            res = qFunc.putJson(json_file=json_file, json_dic=json_dic, )
