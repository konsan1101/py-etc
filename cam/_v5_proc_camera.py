...
class proc_camera:
    ...
    def begin(self, ):
        ...
        qRiKi.statusSet(self.fileRun, False)
        qRiKi.statusSet(self.fileRdy, False)
        qRiKi.statusSet(self.fileBsy, False)
    ...
    def main_proc(self, cn_r, cn_s, ):
        ...
        qRiKi.statusSet(self.fileRun, True)
        self.proc_beat = time.time()
        ...
        while (self.proc_step == '5'):
            ...
            if (self.camDev.isdigit()):
                if (capture is None):
                    if ((qRiKi.statusCheck(qBusy_dev_cam) == False) \
                    or  (qRiKi.statusCheck(qRdy__v_sendkey) == True)):

                        dev = self.camDev
                        if (qRiKi.statusCheck(qRdy__v_mirror) == True):
                            dev = '0'
                        ...
                        # ビジー設定 (ready)
                        if (qRiKi.statusCheck(self.fileBsy) == False):
                            qRiKi.statusSet(self.fileBsy, True)
                            if (str(self.id) == '0'):
                                qRiKi.statusSet(qBusy_v_inp, True)

                if  (not capture is None):
                    if ((qRiKi.statusCheck(qBusy_dev_cam) == True) \
                    and (qRiKi.statusCheck(qRdy__v_sendkey) == False)):
                        capture.release()
                        capture = None
                        ...
                        qRiKi.statusSet(self.fileBsy, False)
                        if (str(self.id) == '0'):
                            qRiKi.statusSet(qBusy_v_inp, False)

            # レディ設定
            if (not capture is None) and (not os.path.exists(self.fileRdy)):
                qRiKi.statusSet(self.fileRdy, True)
                ...
                if (ret == True):
                    ...
                    if (float(self.camSquare) != 0):
                        ...
                        or (self.runMode == 'camera'):
                            if  (qRiKi.statusCheck(qBusy_d_rec) == False):
            ...
            # アイドリング
            slow = False
            if  (qRiKi.statusCheck(qBusy_dev_cpu) == True):
                slow = True
            if  (qRiKi.statusCheck(qBusy_dev_cam  ) == True):
                slow = True
            if  (qRiKi.statusCheck(qBusy_d_play   ) == True) \
            or  (qRiKi.statusCheck(qBusy_d_browser) == True):
                slow = True
...
if __name__ == '__main__':
    ...
    # ビジー解除 (!ready)
    qRiKi.statusSet(qBusy_dev_cam, False)
